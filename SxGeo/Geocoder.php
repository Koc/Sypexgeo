...
class Geocoder
{
    ...
    public function __construct($db_file = 'SxGeo.dat', $type = self::MODE_FILE)
    {
        $this->fh = fopen($db_file, 'rb');
        // Сначала убеждаемся, что есть файл базы данных
        $header = fread($this->fh, 32);
        if (substr($header, 0, 3) != 'SxG') {
            throw new \InvalidArgumentException(sprintf('File "%" not exists', $db_file));
        }
        $info = unpack(
        ...
        if ($info['b_idx_len'] * $info['m_idx_len'] * $info['range'] * $info['db_items'] * $info['time'] * $info['id_len'] == 0) {
            throw new \InvalidArgumentException(sprintf('File "%" in wrong format', $db_file));
        }
        ...
        $this->batch_mode = $type & self::MODE_BATCH;
        $this->memory_mode = $type & self::MODE_MEMORY;
    ...
    public function get_num($ip)
    {
        ...
        if ($ip1n == 0 || $ip1n == 10 || $ip1n == 127 || $ip1n >= $this->b_idx_len || false === ($ipn = ip2long(
                $ip
            ))
        ) {
            return false;
